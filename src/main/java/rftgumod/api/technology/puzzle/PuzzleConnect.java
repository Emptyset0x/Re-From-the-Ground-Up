...
import rftgumod.api.inventory.ContainerResearch;
import rftgumod.api.inventory.InventoryCraftingPersistent;
import rftgumod.api.inventory.SlotCrafting;
import rftgumod.api.technology.puzzle.gui.PuzzleGuiConnect;
...
public class PuzzleConnect  implements IPuzzle {
    ...
    private static boolean connects(ItemPredicate predicate, ItemStack stack) {
            ...
            if (predicate.test(recipe.getRecipeOutput())) {
                for (Ingredient ingredient : recipe.getIngredients())
                    if (ingredient.test(stack))
                        return true;
            } else if (RFTGUAPI.stackUtils.isEqual(recipe.getRecipeOutput(), stack))
                for (Ingredient ingredient : recipe.getIngredients())
        ...
        for (Map.Entry<ItemStack, ItemStack> entry : FurnaceRecipes.instance().getSmeltingList().entrySet()) {
            if (predicate.test(entry.getValue()) && RFTGUAPI.stackUtils.isStackOf(entry.getKey(), stack))
                return true;
            if (RFTGUAPI.stackUtils.isEqual(entry.getValue(), stack))
                for (ItemStack s : predicate.getMatchingStacks())
                    if (RFTGUAPI.stackUtils.isStackOf(entry.getKey(), s))
                        return true;
        }
        return false;
    }
    ...
    private boolean fits(ItemStack stack, int index) {
        if (research.left.test(stack) || research.right.test(stack))
            return false;
        for (int i = 0; i < 3; i++)
            if (RFTGUAPI.stackUtils.isEqual(stack, inventory.getStackInSlot(i)))
                return false;
