...
import rftgumod.common.event.PlayerInspectEvent;
...
public class ItemMagnifyingGlass extends Item {
    ...
    public static List<BlockSerializable> getInspected(ItemStack item) {
        ...
        NBTTagList blocks = StackUtils.INSTANCE.getItemData(item).getTagList("RFTGU", Constants.NBT.TAG_COMPOUND);
        ...
        if (player.isSneaking()) {
            if (!world.isRemote) {
                ...
                if (event.isCanceled()) {
                    ...
                    NBTTagList nbt = tag.getTagList("RFTGU", Constants.NBT.TAG_COMPOUND);
                    nbt.appendTag(block.serialize());
                    tag.setTag("RFTGU", nbt);
                }
            }
